...
mask_futures = (df['PitcherTeam']=='MIN_LOT')|(df['PitcherTeam']=='MIN_KTW')|(df['PitcherTeam']=='MIN_HAN')|(df['PitcherTeam']=='KBO_ARM')|(df['PitcherTeam']=='MIN_LGT')|(df['PitcherTeam']=='MIN_SSG')|(df['PitcherTeam']=='MIN_NCD')|(df['PitcherTeam']=='MIN_KIA')|(df['PitcherTeam']=='MIN_DOO')|(df['PitcherTeam']=='MIN_HER')|(df['PitcherTeam']=='MIN_SAM')
df = df[mask_futures]

df_sam = df[df['PitcherTeam'] == 'MIN_SAM']

dfcopy=df.copy()

pitidmap = df_sam[['PitcherId', 'PitName1', 'PitcherThrows']]
pitidmap = pitidmap.drop_duplicates(subset='PitcherId')
...
yimsi=pd.DataFrame(columns=['Cluster', 'ClusterName', 'PitchUID'])

for j in pitid:
    
    ID = j
    name = id_index_pitidmap.loc[j, 'PitName1']
    ...
    if len(tempdf)<=2:
        ...
        for k in uniquecl:
            ...
            ax.scatter(temp['HorzBreak'], temp['InducedVertBreak'], alpha=0.2, c=coloring(k), label=uniquecl)
            ax.legend(uniquecl)
            
            ax.set_xlabel('수평 무브먼트')
            ax.set_ylabel('수직 무브먼트')
            
            ax.plot([0,0], [-60,80], linewidth=1, color='black')
            ax.plot([-60,60], [0,0], linewidth=1, color='black')
        ...
        plt.savefig("C:\\Users\\LIONS\\.spyder-py3\\7월 4주\\2군 투수 시각화\\%s_%d_by_Cluster.png" % (name, ID), dpi=100)
       
    
            
        Y.to_csv('C:\\Users\\LIONS\\.spyder-py3\\7월 4주\\2군 투수 시각화\\%s %d.csv' % (name, ID), index=False, encoding='cp949')
        ...
        for k in uniqueclname:
            ...
            ax.scatter(temp['HorzBreak'], temp['InducedVertBreak'], alpha=0.2, c=coloring2(k), label=k)
            ax.legend()
        ...
        for k in uniqueclname:
            ...
            df_side = df[df['PitcherThrows']==throws]
            df_pitchtype = df_side[df_side['KorPitchType'] == k]
            meanx = df_pitchtype['HorzBreak'].mean()
            meany = df_pitchtype['InducedVertBreak'].mean()
            ax.scatter(meanx, meany, c=coloring2(k), edgecolor='black', marker='o', s=150, label="%s 2군 평균"%k)
            ax.legend()
        
        plt.title("\n" + name + " 무브먼트 별 구종 분류\n", fontsize = 20)
        plt.savefig("C:\\Users\\LIONS\\.spyder-py3\\7월 4주\\2군 투수 시각화\\%s_%d_by_Cluster_Name.png" % (name, ID), dpi=100)
...
dfcopy.to_csv('C:\\Users\\LIONS\\.spyder-py3\\7월 4주\\Classified.csv', index=False, encoding='cp949')
